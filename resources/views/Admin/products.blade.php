...
@section('title', config('adminlte.title').' | '.__('auth.products'))
                    ...
                    <table id="example1" class="table table-bordered text-center">
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td>{{$index+1}}</td>
                          <td>{{$r->name}}</td>
                          <td>{{$r->getCategory->name}}</td>
                          <td>{{$r->price}}</td>
                          <td>
                            @if($roles->e=="0"&&$roles->d=="0") <span class="badge badge-info">No Permissions</span> @endif
                            @if($roles->e=="1")<button data-rowid="{{$r->id}}" data-name="{{$r->name}}" data-price="{{$r->price}}" data-description="{{$r->description}}" data-image="{{$r->image}}" data-categoryid="{{$r->category_id}}" class="btn btn-xs btn-warning editButton"><i class="fa fa-edit"></i> {{ __('auth.edit') }}</button>@endif
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="category_id" id="category_id" required >
                     <option value="">-- {{ __('auth.select') }} {{ __('auth.category') }} --</option>
                ...
                <div class="form-group">
                    <label>{{ __('auth.price') }}:</label>
                    <input type="number" class="form-control" name="price" id="price" placeholder="{{ __('auth.price') }}" required>
                </div>

                <div class="form-group">
                    <label>{{ __('auth.description') }}:</label>
                    <textarea class="form-control" name="description" id="description" placeholder="{{ __('auth.description') }}" required> </textarea>
                </div>
    ...
    <script>
      $(function () {
        ...
        $(".editButton").on("click", function(){
            ...
            $("#editForm #price").val($(this).data("price"));
            $("#editForm #description").val($(this).data("description"));
            $("#editForm #category_id").val($(this).data("categoryid"));
            $("#editForm").modal("show");
        });
